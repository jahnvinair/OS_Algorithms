...
void completionTime(Process processes[], int n) {
    int current_time = 0;
...
void turnaroundTime(Process processes[], int n) {
    for (int i = 0; i < n; i++) {
        processes[i].tat = processes[i].ct - processes[i].at;
    }
}

void waitingTime(Process processes[], int n) {
    for (int i = 0; i < n; i++) {
